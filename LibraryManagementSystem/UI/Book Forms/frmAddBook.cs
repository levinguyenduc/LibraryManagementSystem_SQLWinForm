...
namespace LibraryManagementSystem.UI.Book_Forms
{
    public partial class frmAddBook : WeifenLuo.WinFormsUI.DockContent
    {
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                ep.SetError(txtBookName, "Book Name cannot be empty!");
                txtBookName.Focus();
                return;
            }
            if (txtTitle.Text.Trim().Length == 0)
            {
                ep.SetError(txtTitle, "Title cannot be empty!");
                txtTitle.Focus();
                return;
            }
            if (txtAuthor.Text.Trim().Length == 0)
            {
                ep.SetError(txtAuthor, "Author cannot be empty!");
                txtAuthor.Focus();
                return;
            }
            if (txtEdition.Text.Trim().Length == 0)
            {
                ep.SetError(txtEdition, "Edition cannot be empty!");
                txtEdition.Focus();
                return;
            }
            if (txtNoOfCopies.Text.Trim().Length == 0)
            {
                ep.SetError(txtNoOfCopies, "Number of Copies cannot be empty!");
                txtNoOfCopies.Focus();
                return;
            }
            if (txtPrice.Text.Trim().Length == 0)
            {
                ep.SetError(txtPrice, "Price cannot be empty!");
                txtPrice.Focus();
                return;
            }
            ...
            if (!BooksHelper.HaveBook(txtBookName.Text, txtAuthor.Text, txtAuthor.Text))
            {
                ...
                MessageBox.Show("Book Addition Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show("The book you are trying to add already exists in the system!", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Book Update Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
