...
namespace LibraryManagementSystem.UI.User_Forms
{
    public partial class frmAddUser : WeifenLuo.WinFormsUI.DockContent
    {
        ...
        {
            epUserName.Clear();
            bool check1 = SessionsHelper.ControlValidate(txtUserName, "Please enter a User Name.", epUserName);

            epPassword.Clear();
            bool check2 = SessionsHelper.ControlValidate(txtEnterPassword, "Please enter a Password.", epPassword);
            ...
            if (check1 && check2)
            {
                if (!UsersHelper.HaveUserID(txtUserName.Text.ToLower()))
                {
                    ...
                    MessageBox.Show("User addition successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("The User Name you want to add already exists in the system.", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    }
